...
def connection_string() -> str:
    ...
    trusted = os.getenv(
        "CUSTOMER_ANALYTICS_SQL_TRUSTED_CONNECTION",
        "yes",
    )
...
def ler_sql(caminho: Path) -> str:
    ...
    raise UnicodeError(
        f"Codificação SQL não suportada: {caminho}"
    )
...
def validar_seguranca(caminho: Path, conteudo: str) -> None:
    ...
    # Tabelas iniciadas por # são temporárias e existem apenas
    # durante a sessão SQL.
    #
    # Removemos os comandos DROP aplicados a essas tabelas da
    # inspeção antes de procurar operações DROP permanentes.
    sql_inspecao = re.sub(
    ...
    proibicoes = [
        (
            ...
            "DML permanente/destrutivo",
        ),
        (
            r"\bINSERT\s+INTO\s+(?!#)",
            "INSERT em objeto permanente",
        ),
        (
            r"\bSELECT\b[\s\S]*?\bINTO\s+(?!#)",
    ...
    for padrao, descricao in proibicoes:
        if re.search(
            ...
            raise RuntimeError(
                f"Operação bloqueada em {relativo}: {descricao}"
            )

    setup_autorizado = (
        relativo == "sql/00_setup/01_criacao_schemas.sql"
    )

    if (
        re.search(
            r"\b(?:CREATE\s+SCHEMA|EXEC(?:UTE)?)\b",
            sql,
            re.IGNORECASE,
        )
        and not setup_autorizado
    ):
        raise RuntimeError(
            f"CREATE SCHEMA/EXEC não autorizado em {relativo}"
        )

    views = re.findall(
        r"\bCREATE\s+(?:OR\s+ALTER\s+)?VIEW\s+([^\s(]+)",
        sql,
        flags=re.IGNORECASE,
    )
    ...
    for view in views:
        ...
        if (
            not relativo.startswith("sql/03_eda/")
            or not nome.startswith("eda.")
        ):
            raise RuntimeError(
                f"CREATE VIEW não autorizado em "
                f"{relativo}: {view}"
            )
...
def valor_canonico(valor: Any) -> Any:
    ...
    if isinstance(valor, bytes):
        return {
            "bytes_sha256": hashlib.sha256(valor).hexdigest()
        }
...
def resumo_resultado(
    ...
    for linha in linhas:
        ...
        for indice in indices_status:
            if linha[indice] is not None:
                statuses_observados.add(
                    str(linha[indice]).strip()
                )

    statuses_para_gate = (
        set()
        if eh_historico_execucao
        else statuses_observados
    )
    ...
    resumo = {
        ...
        "gate_validacao_aplicado": (
            not eh_historico_execucao
        ),
    }

    return resumo, statuses_para_gate
...
def executar_script(
    ...
    for lote in lotes:
        ...
        while True:
            if cursor.description:
                resumo, statuses = resumo_resultado(
                    cursor,
                    len(resultados) + 1,
                )
                ...
                reprovacoes.update(
                    ...
                    if status.upper() in STATUS_REPROVADOS
                )
    ...
    if reprovacoes:
        ...
        registro["erro"] = (
            f"Status lógico reprovado: {encontrados}"
        )

        raise ValidacaoSQLReprovada(
            registro["erro"],
            registro,
        )
...
def salvar_registro(
    ...
    destino = OUTPUT_DIR / (
        f"sql_pipeline_"
        f"{inicio.strftime('%Y%m%dT%H%M%S_%fZ')}.json"
    )
    ...
    try:
        ...
        if not scripts:
            raise FileNotFoundError(
                f"Nenhum arquivo SQL encontrado em "
                f"{SQL_ROOT}"
            )
        ...
        for script in scripts:
            relativo = script.relative_to(PROJECT_ROOT)

            print(f"[SQL] {relativo}")
        ...
        registro["duracao_segundos"] = round(
            (fim - inicio).total_seconds(),
            4,
        )

        caminho_log = salvar_registro(
            registro,
            inicio,
        )

        print(f"Log SQL: {caminho_log}")
